...
class Setting(ABC):
    ...
    def background_data(self) -> InteractionMatrix:
        ...
        raise KeyError("Split before trying to access the background_data property.")
    ...
    def evaluation_data_frame(self) -> List[Tuple[InteractionMatrix, InteractionMatrix]]:
        ...
        for dataset_idx in range(self.num_split_set):
            ...
            datasets.append((self._unlabeled_data_frame[dataset_idx].users_in(matching), self._ground_truth_data_frame[dataset_idx].users_in(matching)))
        return datasets
    ...
    def _check_size(self):
        ...
        def check_empty(name, count):
            if count == 0:
                warn(f"{name} resulting from {type(self).__name__} is empty (no interactions).")

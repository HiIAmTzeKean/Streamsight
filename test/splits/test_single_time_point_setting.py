...
@pytest.fixture()
def setting() -> SingleTimePointSetting:
...
class TestSingleTimePointSetting:
    ...
    def test_empty_background_data(self, matrix: InteractionMatrix):
        setting_temp = SingleTimePointSetting(background_t=0, n_seq_data=N_SEQ_DATA, seed=SEED)
        assert setting_temp.t == 0
        with pytest.warns(
            UserWarning, match="Background data resulting from SingleTimePointSetting is empty \(no interactions\)."
        ):
    ...
    def test_no_n_seq_data(self, matrix: InteractionMatrix):
        setting_temp = SingleTimePointSetting(background_t=BACKGROUND_T, n_seq_data=0, seed=SEED)
    ...
    def test_ground_truth_data(self, setting: Setting, matrix: InteractionMatrix):
        expected_ground_truth = pd.DataFrame({"ts": [4, 6, 7, 9], "uid": [2, 4, 3, 5], "iid": [2, 2, 1, 1]})
        setting.split(matrix)

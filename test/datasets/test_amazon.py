...
from streamsight.datasets import AmazonMusicDataset, Dataset
...
def dataset(dataset_path, filename) -> Dataset:
    return AmazonMusicDataset(base_path=dataset_path, filename=filename)
...
class TestAmazonMusicDataset:
    def test_amazon_music_load_dataframe(self, dataset):
        ...
        assert (df.columns == [dataset.ITEM_IX, dataset.USER_IX, dataset.RATING_IX, dataset.TIMESTAMP_IX]).all()
    ...
    def test_amazon_music_add_filter(self, dataset):
        ...
        data = dataset.add_filter(MinItemsPerUser(2, dataset.ITEM_IX, dataset.USER_IX))
        data = dataset.load()

...
class NLastInteractionSplitter(Splitter):
    ...
    def __init__(self, n: int, n_seq_data: int = 1):
        ...
        if n < 1:
            raise ValueError(
                f"n must be greater than 0, got {n}. Values for n < 1 will cause the ground truth data to be empty."
            )
        self.n = n
...
class NPastInteractionTimestampSplitter(TimestampSplitter):
    ...
    def split(self, data: InteractionMatrix) -> Tuple[InteractionMatrix, InteractionMatrix]:
        ...
        if self.include_all_past_data:
            ...
            past_interaction = data.get_users_n_last_interaction(self.n_seq_data, self.t, future_interaction.user_ids)
